...
class _GraphScreenState extends State<GraphScreen> {
  ...
  Widget build(BuildContext context) => Scaffold(
        appBar: AppBar(
          title: const Text('Humidity Trend'),
      ...
      return const Center(
        child: Column(
          ...
          children: [
            ...
            SizedBox(height: 16),
            Text('Search for a location to view Humidity data',
  ...
  Future<void> _fetchWeather(double lat, double lng) async {
    try {
      ...
      // Store all Humidity data points
      ...
      if (tsMap.isEmpty) {
        throw Exception('No Humidity data available for this location');
      }
      ...
      for (var t = _floorHour(start); t.isBefore(_ceilHour(end)); t = t.add(const Duration(hours: 3))) {
        final val = _getHumidityForTimestamp(tsMap, t);
        if (val != null) {
      ...
      setState(() {
        _loading = false;
        _error = 'Failed to load Humidity data: ${e.toString()}';
      });
    }
  }

  // IMPROVED: Better Humidity lookup with wider time window
  double? _getHumidityForTimestamp(Map<DateTime, double> tsMap, DateTime t) {
